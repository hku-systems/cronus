...
register_phys_mem_pgdir(MEM_AREA_IO_SEC, NOUVEAU_CHANNEL_BASE, NOUVEAU_CHANNEL_SIZE);

#define VRAM_BASE 	((long)1 << 32)
#define VHEAP_BASE 	(((long)1 << 32) + ((long)1 << 29))
#define VHEAP_RAM_SIZE 	((long)1 << 29)

#define EXTENDED_TA_RAM_SIZE ((long)1 << 27)
#define VRAM_RAM_SIZE (((long)1 << 29) - (EXTENDED_TA_RAM_SIZE))

register_phys_mem_pgdir(MEM_AREA_RAM_SEC, VRAM_BASE,  VRAM_RAM_SIZE);
register_phys_mem(MEM_AREA_TA_RAM, VRAM_BASE + VRAM_RAM_SIZE, EXTENDED_TA_RAM_SIZE);
