...
fin:
	if (!*va) {
		printk("try mapping a large ta memory\n");
		core_mmu_get_user_va_range(&ta_map_base, &ta_size);
		ta_map_base += (16 << 20);
		ret = map_virt_phys(&ta_map_base, ((long)1 << 32) + (512 - 32) * 1024 * 1024, 32 * 1024 * 1024, 0);
		if (ret) {
			printk("map %lx (%lx) failed\n", ((long)1 << 32) + (512 - 32) * 1024 * 1024, 32 * 1024 * 1024, 32 * 1024 * 1024);
		}
		*va = ta_map_base;
	}
