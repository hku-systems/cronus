...
cudaError_t cudaLaunchKernel(const void *func, dim3 gridDim, dim3 blockDim, void **args, size_t sharedMem, cudaStream_t stream) {
	return cuda::CudaRuntimeInterface::get()->cudaLaunchKernel(func, gridDim, blockDim, args, sharedMem, stream);
}
...
cudaError_t cudaFuncGetParameters(uint32_t *n_par, uint32_t parameters[20],
  const char *entry) {
	return cuda::CudaRuntimeInterface::get()->cudaFuncGetParameters(n_par, parameters, entry);
}
