...
uint64_t vta(
...
#if VTA_DEBUG == 1
  fprintf(stderr, "INFO - Starting FPGA!\n");
#endif
  ...
  if (t == 10000000) {
    fprintf(stderr, "\tWARNING: VTA TIMEOUT!!!!\n");
#if VTA_DEBUG == 1
  } else {
    fprintf(stderr, "INFO - FPGA Finished!\n");
#endif
  }
...
void printParameters() {
  // Some debugging code
  fprintf(stderr, "Size of VTAInsn: %d\n", sizeof(VTAGenericInsn));
  fprintf(stderr, "Size of VTAUop: %d\n", sizeof(VTAUop));
  fprintf(stderr, "VTA_UOP_BUFF_DEPTH: %d\n", VTA_UOP_BUFF_DEPTH);
  fprintf(stderr, "VTA_LOG_UOP_BUFF_DEPTH: %d\n", VTA_LOG_UOP_BUFF_DEPTH);
  fprintf(stderr, "VTA_WGT_BUFF_DEPTH: %d\n", VTA_WGT_BUFF_DEPTH);
  fprintf(stderr, "VTA_LOG_WGT_BUFF_DEPTH: %d\n", VTA_LOG_WGT_BUFF_DEPTH);
  fprintf(stderr, "VTA_INP_BUFF_DEPTH: %d\n", VTA_INP_BUFF_DEPTH);
  fprintf(stderr, "VTA_LOG_INP_BUFF_DEPTH: %d\n", VTA_LOG_INP_BUFF_DEPTH);
  fprintf(stderr, "VTA_ACC_BUFF_DEPTH: %d\n", VTA_ACC_BUFF_DEPTH);
  fprintf(stderr, "VTA_LOG_ACC_BUFF_DEPTH: %d\n", VTA_LOG_ACC_BUFF_DEPTH);
  fprintf(stderr, "VTA_WGT_WORDS: %d\n", VTA_WGT_BUFF_DEPTH*VTA_BLOCK_IN*VTA_BLOCK_OUT);
  fprintf(stderr, "VTA_INP_WORDS: %d\n", VTA_INP_BUFF_DEPTH*VTA_BLOCK_IN);
  fprintf(stderr, "VTA_ACC_WORDS: %d\n", VTA_ACC_BUFF_DEPTH*VTA_BLOCK_OUT);
  fprintf(stderr, "VTA_INS_ELEM_BYTES: %d\n", VTA_INS_ELEM_BYTES);
  fprintf(stderr, "VTA_UOP_ELEM_BYTES: %d\n", VTA_UOP_ELEM_BYTES);
  fprintf(stderr, "VTA_INP_ELEM_BYTES: %d\n", VTA_INP_ELEM_BYTES);
  fprintf(stderr, "VTA_WGT_ELEM_BYTES: %d\n", VTA_WGT_ELEM_BYTES);
  fprintf(stderr, "VTA_ACC_ELEM_BYTES: %d\n", VTA_ACC_ELEM_BYTES);
  fprintf(stderr, "VTA_BLOCK_IN: %d\n", VTA_BLOCK_IN);
  fprintf(stderr, "VTA_BLOCK_OUT: %d\n", VTA_BLOCK_OUT);
}
...
void printInstruction(int num_insn, VTAGenericInsn *insns) {
  ...
  fprintf(stderr, "DEBUG - There are %u instructions\n", num_insn);
  for (int i = 0; i < num_insn; i++) {
    // Fetch instruction and decode opcode
    c.generic = insns[i];
    fprintf(stderr, "DEBUG - INSTRUCTION %u: ", i);
    if (c.mem.opcode == VTA_OPCODE_LOAD || c.mem.opcode == VTA_OPCODE_STORE) {
      // Print instruction field information
      if (c.mem.opcode == VTA_OPCODE_LOAD) {
        fprintf(stderr, "LOAD ");
        if (c.mem.memory_type == VTA_MEM_ID_UOP) fprintf(stderr, "UOP\n");
        if (c.mem.memory_type == VTA_MEM_ID_WGT) fprintf(stderr, "WGT\n");
        if (c.mem.memory_type == VTA_MEM_ID_INP) fprintf(stderr, "INP\n");
        if (c.mem.memory_type == VTA_MEM_ID_ACC) fprintf(stderr, "ACC\n");
      }
      if (c.mem.opcode == VTA_OPCODE_STORE) {
        fprintf(stderr, "STORE ACC\n");
      }
      fprintf(stderr, "\tdep - pop prev: %d, pop next: %d, push prev: %d, push next: %d\n",
      ...
      fprintf(stderr, "\treset_out: %d\n", static_cast<int>(c.gemm.reset_reg));
      fprintf(stderr, "\touter loop - iter: %d, acc: %d, inp: %d, wgt: %d\n",
      ...
      fprintf(stderr, "\tinner loop - iter: %d, acc: %d, inp: %d, wgt: %d\n",
  ...
  fprintf(stderr, "DEBUG - l2g_queue = %d, g2l_queue = %d\n", l2g_queue, g2l_queue);
  fprintf(stderr, "DEBUG - s2g_queue = %d, g2s_queue = %d\n", s2g_queue, g2s_queue);
}

// Helper function: Print micro-ops status
void printMicroOp(int num_uop, VTAUop *uops) {
  // Iterate over all micro ops
  fprintf(stderr, "DEBUG - There are %u micro-ops\n", num_uop);
  for (int i = 0; i < num_uop; i++) {
    // Read micro-op
    fprintf(stderr, "DEBUG - UOP %u: ", i);
    fprintf(stderr, "acc=%u, inp= %u, wgt=%u\n", uops[i].dst_idx, uops[i].src_idx, uops[i].wgt_idx);
  }
}
...
int alu_test(int opcode, bool use_imm, int batch, int vector_size, bool uop_compression) {
  ...
  fprintf(stderr, "=====================================================================================\n");
  fprintf(stderr, "INFO - ALU test of %s: batch=%d, vector_size=%d, uop_compression=%d\n",
  ...
  int insn_idx = 0;
  insn_buf[insn_idx++] =
      get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_UOP, 0, DRAM_BASE, uop_size, 0, 0, 0, 0);
  for (int b = 0; b < batch; b += VTA_BATCH) {
    insn_buf[insn_idx++] = get2DLoadStoreInsn(
        VTA_OPCODE_LOAD,                   // opcode
        VTA_MEM_ID_ACC,                    // vector size
        0,                                 // sram offset
        DRAM_BASE + b / VTA_BATCH * tx_size * input_sets,  // dram offset
        1,                                 // y size
    ...
    insn_buf[insn_idx++] = get2DLoadStoreInsn(
        ...
        0,                                 // sram offset
        DRAM_BASE + b / VTA_BATCH * tx_size,           // dram offset
        1,                                 // y size
        tx_size,                           // x size
        tx_size,                           // x stride
        0,                                 // y pad
  ...
  // Report on timining
  fprintf(stderr, "INFO - Synchronization time: %.3fms\n", static_cast<float>(t_fpga) / 1E6);
  fprintf(stderr, "INFO - Throughput: %.3fGOps/s\n", static_cast<float>(vector_size * batch) / t_fpga);
#else
  // Invoke the VTA
  vta(ins_size,
  ...
  for (int i = 0; i < batch; i++) {
    for (int j = 0; j < vector_size; j++) {
      if (outputs_ref[i][j] != outputs[i][j]) {
        err++;
#if VTA_DEBUG == 1
        fprintf(stderr, "DEBUG - %d, %d: expected 0x%x but got 0x%x\n", i, j,
  ...
  */
  ...
  if (err == 0) {
    fprintf(stderr, "INFO - ALU test successful!\n");
    return 0;
  } else {
    fprintf(stderr, "INFO - ALU test failed, got %d errors!\n", err);
    return -1;
  }
}
...
int blocked_gemm_test(int batch, int channels, int block, bool uop_compression,
  ...
  fprintf(stderr, "=====================================================================================\n");
  fprintf(stderr, "INFO - Blocked GEMM test: batch=%d, channels=%d, block=%d, uop_comp=%d, vt=%d\n",
  ...
  for (int i = 0; i < batch; i += block) {
    ...
    for (int j = 0; j < out_feat; j += block) {
      ...
      insn_buf[insn_idx++] = get2DLoadStoreInsn(
          ...
          VTA_MEM_ID_ACC,                                     // type
          0,                                                  // sram offset
          DRAM_BASE + (i / VTA_BATCH * out_feat + j) / VTA_BLOCK_OUT,     // dram offset
      ...
      for (int k = 0; k < in_feat; k += block * virtual_threads) {
        for (int l = 0; l < block * virtual_threads; l += block) {
          ...
          insn_buf[insn_idx++] = get2DLoadStoreInsn(
              ...
              DRAM_BASE + (j / VTA_BLOCK_OUT * in_feat + k + l) / VTA_BLOCK_IN,  // dram offset
          ...
          insn_buf[insn_idx++] = get2DLoadStoreInsn(
              ...
              DRAM_BASE + (i / VTA_BATCH * in_feat + k + l) / VTA_BLOCK_IN,  // dram offset
              block / VTA_BATCH,                              // y size
              block / VTA_BLOCK_IN,                           // x size
              in_feat / VTA_BLOCK_IN,                         // x stride
              0,                                              // y pad
  ...
  uint64_t t_fpga = vta(ins_size,
                        ...
                        output_buf);
  // Report on timining
  fprintf(stderr, "INFO - Synchronization time: %.3lfms\n", static_cast<float>(t_fpga) / 1E6);
  fprintf(stderr, "INFO - Throughput: %.3lfGOPs/s\n",
         static_cast<float>(batch) * in_feat * out_feat * 2 / t_fpga);
  elapsed_time_ms = batch * in_feat * out_feat * 2 / t_fpga;
#else
  // Invoke the VTA
  vta(ins_size,
      (volatile insn_T *) insn_buf,
  ...
  */
  ...
  if (err == 0) {
    fprintf(stderr, "INFO - Blocked GEMM test successful!\n");
    return 0;
  } else {
    fprintf(stderr, "INFO - Blocked GEMM test failed, got %d errors!\n", err);
    return -1;
  }
}
...
int gemm_test(int batch, int in_channels, int out_channels, bool uop_compression) {
  ...
  fprintf(stderr, "=====================================================================================\n");
  fprintf(stderr, "INFO - Blocked GEMM test: batch=%d, in_channels=%d, out_channels=%d, uop_comp=%d\n",
