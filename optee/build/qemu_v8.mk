...
run-only-default:
	...
	cd $(BINARIES_PATH) && $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		-nographic \
		-serial telnet:0.0.0.0:54310,server -serial telnet:0.0.0.0:54311,server \
		-smp $(QEMU_SMP) \
		--accel tcg,thread=multi \
		-gdb tcp::12335 -S -machine virt,secure=on -cpu cortex-a57 \
...
run-only-bg:
	ln -sf $(ROOT)/out-br/images/rootfs.cpio.gz $(BINARIES_PATH)/
	$(call check-terminal)
	$(call wait-for-ports,54310,54311)
	$(call run-help)
	cd $(BINARIES_PATH) && $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		-nographic \
		-serial telnet:0.0.0.0:54310 -serial telnet:0.0.0.0:54311 \
		-smp $(QEMU_SMP) \
		--accel tcg,thread=multi \
		-gdb tcp::12335 -S -machine virt,secure=on -cpu cortex-a57 \
		-d unimp -semihosting-config enable=on,target=native \
...
run-only-debug:
	...
	cd $(BINARIES_PATH) && $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		-nographic \
		-serial telnet:0.0.0.0:54320 -serial telnet:0.0.0.0:54321 \
		-smp $(QEMU_SMP) \
		--accel tcg,thread=multi \
		-gdb tcp::12335 -S -machine virt,secure=on -cpu cortex-a57 \
		-d unimp -semihosting-config enable=on,target=native \
...
run-only:
	...
	$(call check-terminal)
	$(call run-help)
	$(call launch-terminal,54310,"Normal World")
	$(call launch-terminal,54311,"Secure World")
	$(call wait-for-ports,54310,54311)
	cd $(BINARIES_PATH) && $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		-nographic \
		-serial tcp:localhost:54310 -serial tcp:localhost:54311 \
		-smp $(QEMU_SMP) \
		--accel tcg,thread=multi \
		-gdb tcp::12335 -S -machine virt,secure=on -cpu cortex-a57 \
		-d unimp -semihosting-config enable=on,target=native \
		...
		-device vfio-pci,host=3b:00.0,bus=pcie.0 \
		$(QEMU_EXTRA_ARGS)
# -nic user,hostfwd=tcp::2345-:22
#  -device vfio-pci,host=af:00.0 
#  -kernel Image -no-acpi \
# -append 'console=ttyAMA0,38400 keep_bootcon root=/dev/vda1'	
# secure=on,dumpdtb=trustzone.dtb
# -device edu,bus=pcie.0 \
...
run-remote-bg:
	...
	$(call check-terminal)
	$(call run-help)
	cd $(BINARIES_PATH) && nohup $(QEMU_BUILD)/aarch64-softmmu/qemu-system-aarch64 \
		-nographic \
		-serial telnet:0.0.0.0:54310,server -serial telnet:0.0.0.0:54311,server \
		-smp $(QEMU_SMP) \
		--accel tcg,thread=multi \
