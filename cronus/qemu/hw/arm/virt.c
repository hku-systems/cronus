...
AddressSpace secure_address_space_memory;
...
static const MemMapEntry base_memmap[] = {
    ...
    [VIRT_PCIE_MMIO] =          { 0x10000000, 0x0eff0000 },
    [VIRT_PCIE_PIO] =           { 0x1eff0000, 0x00010000 },
    [VIRT_PCIE_ECAM] =          { 0x1f000000, 0x01000000 },
    [VIRT_SECURE_PCIE_MMIO] =   { 0x20000000, 0x0eff0000 },
    [VIRT_SECURE_PCIE_PIO] =    { 0x2eff0000, 0x00010000 },
    [VIRT_SECURE_PCIE_ECAM] =   { 0x2f000000, 0x01000000 },
    /* Actual RAM size depends on initial RAM and device memory settings */
    [VIRT_MEM] =                { GiB, LEGACY_RAMLIMIT_BYTES },
    [VIRT_SECURE_VMEM] =        { 0, SECURE_VRAM_LEN },
};
...
static MemMapEntry extended_memmap[] = {
    ...
    [VIRT_SECURE_HIGH_PCIE_ECAM] =     { 0x0, 256 * MiB },
    [VIRT_SECURE_HIGH_PCIE_MMIO] =     { 0x0, 512 * GiB },
};
                                ...
                                int first_irq, const char *nodename)
{
    ...
    if (first_irq == 96) {
        irq_level = GIC_FDT_IRQ_FLAGS_EDGE_LO_HI;
    }
...
static void create_pcie(VirtMachineState *vms)
{
    ...
    vms->bus = pci->bus;
    pci->bus->mem_address_space = &address_space_memory;
...
AddressSpace null_address_space_memory;
static void create_secure_pcie(VirtMachineState *vms, MemoryRegion *sm)
{
    hwaddr base_mmio = vms->memmap[VIRT_SECURE_PCIE_MMIO].base;
    hwaddr size_mmio = vms->memmap[VIRT_SECURE_PCIE_MMIO].size;
    hwaddr base_mmio_high = vms->memmap[VIRT_SECURE_HIGH_PCIE_MMIO].base;
    hwaddr size_mmio_high = vms->memmap[VIRT_SECURE_HIGH_PCIE_MMIO].size;
    hwaddr base_pio = vms->memmap[VIRT_SECURE_PCIE_PIO].base;
    hwaddr size_pio = vms->memmap[VIRT_SECURE_PCIE_PIO].size;
    hwaddr base_ecam, size_ecam;
    hwaddr base = base_mmio;
    int nr_pcie_buses;
    int irq = vms->irqmap[VIRT_SECURE_PCIE];
    MemoryRegion *mmio_alias;
    MemoryRegion *mmio_reg;
    MemoryRegion *ecam_alias;
    MemoryRegion *ecam_reg;
    ...
    ecam_id = VIRT_SECURE_ECAM_ID(vms->highmem_ecam);
    base_ecam = vms->memmap[ecam_id].base;
    size_ecam = vms->memmap[ecam_id].size;
    nr_pcie_buses = size_ecam / PCIE_MMCFG_SIZE_MIN;
    /* Map only the first size_ecam bytes of ECAM space */
    ecam_alias = g_new0(MemoryRegion, 1);
    ecam_reg = sysbus_mmio_get_region(SYS_BUS_DEVICE(dev), 0);
    memory_region_init_alias(ecam_alias, OBJECT(dev), "pcie-secure-ecam",
                             ecam_reg, 0, size_ecam);
    memory_region_add_subregion(sm, base_ecam, ecam_alias);
    ...
    mmio_alias = g_new0(MemoryRegion, 1);
    mmio_reg = sysbus_mmio_get_region(SYS_BUS_DEVICE(dev), 1);
    memory_region_init_alias(mmio_alias, OBJECT(dev), "pcie-secure-mmio",
                             mmio_reg, base_mmio, size_mmio);
    memory_region_add_subregion(sm, base_mmio, mmio_alias);

    if (vms->highmem) {
        /* Map high MMIO space */
        MemoryRegion *high_mmio_alias = g_new0(MemoryRegion, 1);

        memory_region_init_alias(high_mmio_alias, OBJECT(dev), "pcie-secure-mmio-high",
                                 mmio_reg, base_mmio_high, size_mmio_high);
        memory_region_add_subregion(sm, base_mmio_high,
                                    high_mmio_alias);
    }
    ...
    pci = PCI_HOST_BRIDGE(dev);
    vms->bus = pci->bus;
    pci->bus->mem_address_space = &secure_address_space_memory;
    ...
    nodename = vms->pciehb_nodename = g_strdup_printf("/pcie-secure@%" PRIx64, base);
    qemu_fdt_add_subnode(ms->fdt, nodename);
    qemu_fdt_setprop_string(ms->fdt, nodename,
                            "compatible", "pci-host-ecam-generic");
    qemu_fdt_setprop_string(ms->fdt, nodename, "device_type", "pci");
...
static void create_secure_vram(VirtMachineState *vms,
                              MemoryRegion *secure_sysmem,
                              MemoryRegion *secure_tag_sysmem)
{
    MemoryRegion *secram = g_new(MemoryRegion, 1);
    char *nodename;
    hwaddr base = vms->memmap[VIRT_SECURE_VMEM].base;
    hwaddr size = vms->memmap[VIRT_SECURE_VMEM].size;
    MachineState *ms = MACHINE(vms);

    memory_region_init_ram(secram, NULL, "virt.secure-vram", size,
                           &error_fatal);
    memory_region_add_subregion(secure_sysmem, base, secram);

    nodename = g_strdup_printf("/secvram@%" PRIx64, base);
    qemu_fdt_add_subnode(ms->fdt, nodename);
    qemu_fdt_setprop_string(ms->fdt, nodename, "device_type", "memory");
    qemu_fdt_setprop_sized_cells(ms->fdt, nodename, "reg", 2, base, 2, size);
    qemu_fdt_setprop_string(ms->fdt, nodename, "status", "disabled");
    ...
    if (secure_tag_sysmem) {
        create_tag_ram(secure_tag_sysmem, base, size, "mach-virt.vsecure-tag");
    }
...
static void virt_set_memmap(VirtMachineState *vms)
{
    ...
    hwaddr secure_vram_base, secure_vram_size;
    int i;
    ...
    /* 
     * We use the lower memory for another large pool of secure memory
     */
    secure_vram_base = ROUND_UP(vms->memmap[VIRT_MEM].base + ms->ram_size, GiB);
    secure_vram_size = SECURE_VRAM_LEN;
    vms->memmap[VIRT_SECURE_VMEM].base = secure_vram_base;
    ...
    device_memory_base = ROUND_UP(secure_vram_base + secure_vram_size, GiB);
    device_memory_size = ms->maxram_size - ms->ram_size - secure_vram_size + ms->ram_slots * GiB;
...
static void machvirt_init(MachineState *machine)
{
    ...
    if (vms->secure) {
        ...
        memory_region_init(secure_sysmem, OBJECT(machine), "secure-memory",
                           UINT64_MAX);
        address_space_init(&secure_address_space_memory, secure_sysmem, "secure-memory");
        memory_region_add_subregion_overlap(secure_sysmem, 0, sysmem, -1);
        null_sysmem = g_new(MemoryRegion, 1);
        memory_region_init(null_sysmem, OBJECT(machine), "null-memory",
                        UINT64_MAX);
        address_space_init(&null_address_space_memory, null_sysmem, "null-memory");
    }
    ...
    if (vms->secure) {
        ...
        create_secure_vram(vms, secure_sysmem, secure_tag_sysmem);
