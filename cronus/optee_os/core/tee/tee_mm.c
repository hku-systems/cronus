...
#endif
    if (ret == NULL)
        panic("error in %lx (%d) %s\n", phys_addr, size, __FUNCTION__);
    return ret;
}

void iounmap(volatile void __iomem *addr) {}

extern uint32_t map_virt_phys(uintptr_t *virt_addr, uintptr_t phys_addr, size_t size, int prot);

int remap_pfn_range(struct vm_area_struct * s, unsigned long addr,
			unsigned long pfn, unsigned long size, pgprot_t pgp) {
    return map_virt_phys(&addr, __pfn_to_phys(pfn), size, 0);
}
